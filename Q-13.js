...
// use brand as the argument passed to the constructor function
// assign this.carname property to the value of brand when a car object is instantiated
class Car {
    constructor(brand) {
      this.carname = brand;
    }
    // create a method that will return a message with the brand of the car using this.carname property
    present() {
...
class Model extends Car {
    ...
    // uses return value of this.present() method inherited from the parent class
    // and the value of this.model property - passed to the constructor when a Model object is instantiated
    show() {
        return this.present() + ', it was made in ' + this.model;
    }
}
  
// Assigning an array of strings to the variable makes
let makes = ["Ford", "Holden", "Toyota"]

// using the Array.from method to return an array and assign it to the variable models
// Array.from is taking in the argument new Array(40) which creates an array of 40 elements, all of which are empty
...
// this will iterate through each element -  models[1980, 1981, 1982...2019]
for (model of models) {
    // call randomIntFromInterval()
    // 0 is argument for min, (makes.length-1 is the last element) passed as the max value
    // will return a random index between 0 and 2 (last element of the array)
    // the value of that index in the makes[] will be passed to the make variable
    ...
    // model = models[randomIntFromInterval(0,makes.length-1)]   < -----------incorrect line of code

    // Now the max value is set to the last element of the models array (39)
    model = models[randomIntFromInterval(0,models.length-1)] //  < -----------refactored line of code
